...
def send_email(recipient, subject, html_message, text_message):
    ...
    try:
        ...
        raise SendEmailError(
            "Flask-Mail has not been installed. Use 'pip install Flask-Mail' to install Flask-Mail.")
...
def send_confirm_email_email(user, user_email, confirm_email_link):
    ...
    if not user_manager.enable_email:
        return
    if not user_manager.send_registered_email and \
       not user_manager.enable_confirm_email:
        return
...
def send_password_changed_email(user):
    ...
    if not user_manager.enable_email:
        return
    if not user_manager.send_password_changed_email:
        return
...
def send_username_changed_email(user):  # pragma: no cover
    ...
    if not user_manager.enable_email:
        return
    if not user_manager.send_username_changed_email:
        return
...
def send_invite_email(user, accept_invite_link):
    ...
    # Send email message using Flask-Mail
    user_manager.send_email_function(
        user.email, subject, html_message, text_message)
