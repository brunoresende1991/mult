""" This file defines and validates Flask-User forms. Forms are
...
try:
    ...
    from flask_wtf import Form as FlaskForm     # Fallback to Flask-WTF v0.12 or older # noqa

from wtforms import BooleanField, HiddenField, PasswordField, SubmitField
from wtforms import validators, ValidationError, StringField
...
def theme_validator(form, field):
    theme = field.data
    if theme not in ['red', 'blue', 'green']:
        raise ValidationError(_("Theme must be red, blue or green. "
...
def username_validator(form, field):
    ...
    valid_chars = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-._'  # noqa
    chars = list(username)
    for char in chars:
        if char not in valid_chars:
            raise ValidationError(_("Username may only contain letters, "
                                    "numbers, '-', '.' and '_'"))
...
def unique_tenant_validator(form, field):
    """ Tenant must be unique"""
    user_manager = current_app.user_manager
    if not user_manager.tenant_is_available(field.data):
        raise ValidationError(_('This Tenant is already in use.'
                                ' Please try another one.'))
...
class ChangePasswordForm(FlaskForm):
    ...
    retype_password = PasswordField(_('Retype New Password'), validators=[
        validators.EqualTo('new_password', message=_('New Password and Retype '
    ...
    def validate(self):
        ...
        if not current_user or not user_manager.verify_password(
           self.old_password.data, current_user):
...
class ChangeThemeForm(FlaskForm):
    new_theme = StringField(_('New Theme'), validators=[
        validators.DataRequired(_('Theme is required')),
        theme_validator,
    ])
    old_tenant = StringField(_('Old Tenant'), validators=[
        validators.DataRequired(_('Tenant is required')),
    ])
    old_password = PasswordField(_('Old Password'), validators=[
        validators.DataRequired(_('Old Password is required')),
    ])
    next = HiddenField()
    submit = SubmitField(_('Change theme'))

    def validate(self):
        user_manager = current_app.user_manager

        # Validate field-validators
        if not super(ChangeThemeForm, self).validate():
            return False
...
class AddTenantForm(FlaskForm):
    new_tenant = StringField(_('New Tenant'), validators=[
        validators.DataRequired(_('Tenant is required')),
        unique_tenant_validator,
    ])
    old_password = PasswordField(_('Old Password'), validators=[
        validators.DataRequired(_('Old Password is required')),
    ])
    next = HiddenField()
    submit = SubmitField(_('Add tenant'))

    def validate(self):
        user_manager = current_app.user_manager

        # Validate field-validators
        if not super(AddTenantForm, self).validate():
            return False
...
class EditTenantForm(FlaskForm):
    ...
    new_alias = StringField(_('New Alias'), validators=[
        validators.DataRequired(_('Alias is required')),
        unique_tenant_validator,
    ])
    old_password = PasswordField(_('Old Password'), validators=[
        validators.DataRequired(_('Old Password is required')),
    ...
    next = HiddenField()
    submit = SubmitField(_('Edit tenant'))

    def validate(self):
        user_manager = current_app.user_manager

        # Validate field-validators
        if not super(EditTenantForm, self).validate():
            return False
...
class AddTenantUserForm(FlaskForm):
    ...
    new_user = StringField(_('New User'), validators=[
        validators.DataRequired(_('User is required')),
    ])
    ...
    next = HiddenField()
    submit = SubmitField(_('Add tenant user'))

    def validate(self):
        user_manager = current_app.user_manager

        # Validate field-validators
        if not super(AddTenantUserForm, self).validate():
            return False
...
class RemoveTenantUserForm(FlaskForm):
    old_tenant = StringField(_('Old Tenant'), validators=[
        validators.DataRequired(_('Tenant is required')),
    ])

    old_user = StringField(_('Old User'), validators=[
        validators.DataRequired(_('User is required')),
    ])
    ...
    next = HiddenField()
    submit = SubmitField(_('Remove tenant user'))

    def validate(self):
        user_manager = current_app.user_manager

        # Validate field-validators
        if not super(RemoveTenantUserForm, self).validate():
            return False
...
class DeleteTenantForm(FlaskForm):
    ...
    next = HiddenField()
    submit = SubmitField(_('Delete tenant'))

    def validate(self):
        user_manager = current_app.user_manager

        # Validate field-validators
        if not super(DeleteTenantForm, self).validate():
            return False
    ...
    def validate(self):
        ...
        for v in self.new_username.validators:
            if v == user_manager.username_validator:
                has_been_added = True
        if not has_been_added:
            self.new_username.validators.append(
...
class ForgotPasswordForm(FlaskForm):
    ...
    def validate_email(form, field):
        ...
        if user_manager.show_username_email_does_not_exist:
            ...
            if not user:
                raise ValidationError(_('%(username_or_email)s does not exist',
                                        username_or_email=_('Email')))
...
class LoginForm(FlaskForm):
    ...
    def validate(self):
        ...
        if user and user_manager.get_password(user) and \
        ...
        # Hide 'username/email does not exist error message for additional security # noqa
        else:
...
class RegisterForm(FlaskForm):
    ...
    retype_password = PasswordField(_('Retype Password'), validators=[
        validators.EqualTo('password', message=_(
...
class ResetPasswordForm(FlaskForm):
    new_password = PasswordField(_('New Password'), validators=[
        validators.DataRequired(_('New Password is required'))])
    retype_password = PasswordField(_('Retype New Password'), validators=[
        validators.EqualTo('new_password', message=_(
            'New Password and Retype Password did not match'))])
    next = HiddenField()
    submit = SubmitField(_('Change password'))
