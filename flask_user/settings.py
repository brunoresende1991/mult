...
def set_default_settings(user_manager, app_config):
    """ Set default app.config settings,
    but only if they have not been set before """
    # define short names
    um = user_manager
    ...
    obsoleted_enable_retype_passwords = sd(
    ...
    um.enable_forgot_password = sd(
        'USER_ENABLE_FORGOT_PASSWORD', um.enable_email)
    um.enable_login_without_confirm_email = sd(
        'USER_ENABLE_LOGIN_WITHOUT_CONFIRM_EMAIL', False)
    um.enable_multiple_emails = sd('USER_ENABLE_MULTIPLE_EMAILS', False)
    um.enable_register = sd(
        'USER_ENABLE_REGISTER', obsoleted_enable_registration)
    um.enable_remember_me = sd('USER_ENABLE_REMEMBER_ME', True)
    um.enable_retype_password = sd(
        'USER_ENABLE_RETYPE_PASSWORD', obsoleted_enable_retype_passwords)
    um.enable_username = sd('USER_ENABLE_USERNAME', obsoleted_enable_usernames)

    # Set default settings
    um.auto_login = sd('USER_AUTO_LOGIN', True)
    um.auto_login_after_confirm = sd(
        'USER_AUTO_LOGIN_AFTER_CONFIRM', um.auto_login)
    um.auto_login_after_register = sd(
        'USER_AUTO_LOGIN_AFTER_REGISTER', um.auto_login)
    um.auto_login_after_reset_password = sd(
        'USER_AUTO_LOGIN_AFTER_RESET_PASSWORD', um.auto_login)
    um.auto_login_at_login = sd('USER_AUTO_LOGIN_AT_LOGIN', um.auto_login)
    um.confirm_email_expiration = sd(
    ...
    um.reset_password_expiration = sd(
    ...
    um.send_password_changed_email = sd(
        'USER_SEND_PASSWORD_CHANGED_EMAIL', um.enable_email)
    um.send_registered_email = sd(
        'USER_SEND_REGISTERED_EMAIL', um.enable_email)
    um.send_username_changed_email = sd(
        'USER_SEND_USERNAME_CHANGED_EMAIL', um.enable_email)
    um.show_username_email_does_not_exist = sd(
        'USER_SHOW_USERNAME_EMAIL_DOES_NOT_EXIST', um.enable_register)

    # Set default URLs
    um.change_password_url = sd(
        'USER_CHANGE_PASSWORD_URL', '/user/change-password')
    um.change_username_url = sd(
        'USER_CHANGE_USERNAME_URL', '/user/change-username')
    um.change_theme_url = sd(
        'USER_CHANGE_THEME_URL', '/user/change-theme')
    um.add_tenant_url = sd(
        'USER_ADD_TENANT_URL', '/user/add-tenant')
    um.edit_tenant_url = sd(
        'USER_EDIT_TENANT_URL', '/user/edit-tenant')
    um.add_tenant_user_url = sd(
        'USER_ADD_TENANT_USER_URL', '/user/add-tenant-user')
    um.remove_tenant_user_url = sd(
        'USER_REMOVE_TENANT_USER_URL', '/user/remove-tenant-user')
    um.delete_tenant_url = sd(
        'USER_DELETE_TENANT_URL', '/user/delete-tenant')
    um.confirm_email_url = sd(
        'USER_CONFIRM_EMAIL_URL', '/user/confirm-email/<token>')
    um.email_action_url = sd(
        'USER_EMAIL_ACTION_URL', '/user/email/<id>/<action>')
    um.forgot_password_url = sd(
    ...
    um.after_change_password_endpoint = sd(
        'USER_AFTER_CHANGE_PASSWORD_ENDPOINT', home_endpoint)
    um.after_change_username_endpoint = sd(
        'USER_AFTER_CHANGE_USERNAME_ENDPOINT', home_endpoint)
    um.after_change_theme_endpoint = sd(
        'USER_AFTER_CHANGE_THEME_ENDPOINT', home_endpoint)
    um.after_add_tenant_endpoint = sd(
        'USER_AFTER_ADD_TENANT_ENDPOINT', home_endpoint)
    um.after_edit_tenant_endpoint = sd(
        'USER_AFTER_EDIT_TENANT_ENDPOINT', home_endpoint)
    um.after_add_tenant_user_endpoint = sd(
        'USER_AFTER_ADD_TENANT_USER_ENDPOINT', home_endpoint)
    um.after_remove_tenant_user_endpoint = sd(
        'USER_AFTER_REMOVE_TENANT_USER_ENDPOINT', home_endpoint)
    um.after_delete_tenant_endpoint = sd(
        'USER_AFTER_DELETE_TENANT_ENDPOINT', home_endpoint)
    um.after_confirm_endpoint = sd(
        'USER_AFTER_CONFIRM_ENDPOINT', home_endpoint)
    um.after_forgot_password_endpoint = sd(
        'USER_AFTER_FORGOT_PASSWORD_ENDPOINT', home_endpoint)
    ...
    um.after_register_endpoint = sd(
        'USER_AFTER_REGISTER_ENDPOINT', home_endpoint)
    um.after_resend_confirm_email_endpoint = sd(
        'USER_AFTER_RESEND_CONFIRM_EMAIL_ENDPOINT', home_endpoint)
    um.after_reset_password_endpoint = sd(
        'USER_AFTER_RESET_PASSWORD_ENDPOINT', home_endpoint)
    um.after_invite_endpoint = sd('USER_INVITE_ENDPOINT', home_endpoint)
    um.unconfirmed_email_endpoint = sd(
        'USER_UNCONFIRMED_EMAIL_ENDPOINT', home_endpoint)
    um.unauthenticated_endpoint = sd(
        'USER_UNAUTHENTICATED_ENDPOINT', login_endpoint)
    um.unauthorized_endpoint = sd('USER_UNAUTHORIZED_ENDPOINT', home_endpoint)

    # Set default template files
    um.change_password_template = sd(
        'USER_CHANGE_PASSWORD_TEMPLATE', 'flask_user/change_password.html')
    um.change_username_template = sd(
        'USER_CHANGE_USERNAME_TEMPLATE', 'flask_user/change_username.html')
    um.change_theme_template = sd(
        'USER_CHANGE_THEME_TEMPLATE', 'flask_user/change_theme.html')
    um.add_tenant_template = sd(
        'USER_ADD_TENANT_TEMPLATE', 'flask_user/add_tenant.html')
    um.edit_tenant_template = sd(
        'USER_EDIT_TENANT_TEMPLATE', 'flask_user/edit_tenant.html')
    um.add_tenant_user_template = sd(
        'USER_ADD_TENANT_USER_TEMPLATE', 'flask_user/add_tenant_user.html')
    um.remove_tenant_user_template = sd(
        'USER_REMOVE_TENANT_USER_TEMPLATE', 'flask_user/remove_tenant_user.html')  # noqa
    um.delete_tenant_template = sd(
        'USER_DELETE_TENANT_TEMPLATE', 'flask_user/delete_tenant.html')
    um.forgot_password_template = sd(
...
def check_settings(user_manager):
    ...
    """ USER_ENABLE_REGISTER=True must have USER_ENABLE_USERNAME=True or
    USER_ENABLE_EMAIL=True or both."""
    if um.enable_register and not(um.enable_username or um.enable_email):
        raise ConfigurationError(
            'USER_ENABLE_REGISTER=True must have USER_ENABLE_USERNAME=True '
            'or USER_ENABLE_EMAIL=True or both.')
    ...
    if um.require_invitation and not um.enable_invitation:
        raise ConfigurationError(
            'USER_REQUIRE_INVITATION=True must have '
            'USER_ENABLE_INVITATION=True.')
    if um.enable_invitation and not um.db_adapter.UserInvitationClass:
        raise ConfigurationError(
            'USER_ENABLE_INVITATION=True must pass UserInvitationClass '
            'to SQLAlchemyAdapter().')
