...
def change_theme():
    ...
    # Initialize form
    form = user_manager.change_theme_form(request.form)
    safe_next = _get_safe_next_param(
        'next', user_manager.after_change_theme_endpoint)
    form.next_data = safe_next

    # Process valid POST
    if request.method == 'POST' and form.validate():
        new_theme = form.new_theme.data
        old_tenant = form.old_tenant.data
        ...
        tenants = [tenant for tenant in user_auth.tenants if tenant.alias == old_tenant]  # noqa
        if tenants:
            themes = tenants[0].themes
            if themes:
                theme = themes[0]
                db_adapter.update_object(theme, style=new_theme)
            else:
                theme = db_adapter.add_object(db_adapter.Theme, **{
                    "style": new_theme})
                themes.append(theme)
            db_adapter.commit()

            # Send theme_changed signal
            signals.user_changed_theme.send(
                current_app._get_current_object(), user=current_user)

            # Prepare one-time system message
            flash(_(
                "Seu theme foi modificado para '%(theme)s'.",
                theme=new_theme), 'success'
            )
        else:
            # Prepare one-time system message
            flash(_(
                u"Este usuário não possui tenant com o alias '%(tenant)s'.",
                tenant=old_tenant), 'error'
            )
    ...
    # Process GET or invalid POST
    return render(user_manager.change_theme_template, form=form)
...
def add_tenant():
    ...
    # Initialize form
    form = user_manager.add_tenant_form(request.form)
    safe_next = _get_safe_next_param(
        'next', user_manager.after_add_tenant_endpoint)
    form.next.data = safe_next

    # Process valid POST
    if request.method == 'POST' and form.validate():
        new_tenant = form.new_tenant.data
        ...
        if db_adapter.UserAuthClass and hasattr(current_user, 'user_auth'):
            ...
            user_auth = current_user
        db_adapter.add_object(db_adapter.Tenant, **{
            "users": [user_auth], "alias": new_tenant})
        db_adapter.commit()

        # Send tenant_added signal
        signals.user_added_tenant.send(
            current_app._get_current_object(), user=current_user)

        # Prepare one-time system message
        flash(_(
            "Seu tenant ('%(tenant)s') foi adicionado.",
            tenant=new_tenant), 'success'
        )
    ...
    # Process GET or invalid POST
    return render(user_manager.add_tenant_template, form=form)
...
def edit_tenant():
    ...
    # Initialize form
    form = user_manager.edit_tenant_form(request.form)
    safe_next = _get_safe_next_param(
        'next', user_manager.after_edit_tenant_endpoint)
    form.next.data = safe_next

    # Process valid POST
    if request.method == 'POST' and form.validate():
        old_tenant = form.old_tenant.data
        new_alias = form.new_alias.data
        ...
        if len(tenants) > 0:
            tenant = tenants[0]
            db_adapter.update_object(tenant, alias=new_alias)
            db_adapter.commit()

            # Send tenant_edited signal
            signals.user_edited_tenant.send(
                current_app._get_current_object(), user=current_user)

            # Prepare one-time system message
            flash(_(
                "Seu tenant ('%(tenant)s') foi editado.",
                tenant=new_alias), 'success'
            )
            ...
            # Prepare one-time system message
            flash(_(
                u"Este usuário não possui tenant com alias ('%(tenant)s').",
                tenant=old_tenant), 'error'
            )

    # Process GET or invalid POST
    return render(user_manager.edit_tenant_template, form=form)
...
def add_tenant_user():
    ...
    # Initialize form
    form = user_manager.add_tenant_user_form(request.form)
    safe_next = _get_safe_next_param(
        'next', user_manager.after_add_tenant_user_endpoint)
    form.next.data = safe_next
    ...
    if request.method == 'POST' and form.validate():
        ...
        if len(tenants) > 0:
            ...
            if user:
                if user in tenant.users:
                    flash(_(
                        u"Este usuário '%(user)s' já está na lista de usuários deste tenant '%(tenant)s'.",  # noqa
                        tenant=old_tenant, user=new_user), 'error'
                    )
                else:
                    ...
                    # Send tenant_edited signal
                    signals.user_added_tenant_user.send(
                        current_app._get_current_object(), user=current_user)

                    # Prepare one-time system message
                    flash(_(
                        u"Seu tenant '%(tenant)s' foi atualizado com o usuário '%(user)s'.",  # noqa
                        tenant=old_tenant, user=new_user), 'success'
                    )
                ...
                # Prepare one-time system message
                flash(_(
                    u"O usuário '%(user)s' não existe.",  # noqa
                    user=new_user), 'error'
                )
        else:
            # Prepare one-time system message
    ...
    # Process GET or invalid POST
    return render(user_manager.add_tenant_user_template, form=form)
...
def remove_tenant_user():
    ...
    # Initialize form
    form = user_manager.remove_tenant_user_form(request.form)
    safe_next = _get_safe_next_param(
        'next', user_manager.after_edit_tenant_endpoint)
    form.next.data = safe_next
        ...
        if len(tenants) > 0:
            ...
            if user:
                if user in tenant.users:
                    ...
                    # Prepare one-time system message
                    flash(_(
                        u"O usuário '%(user)s' foi removido do tenant '%(tenant)s'.",  # noqa
                        tenant=old_tenant, user=old_user), 'success'
                    )
                    ...
                    # Prepare one-time system message
                    flash(_(
                        u"Este usuário ('%(user)s') não faz parte da lista de usuários deste tenant.",  # noqa
                        user=old_user), 'error'
                    )
            else:
                # Prepare one-time system message
                flash(_(
                    u"Este usuário ('%(user)s') não existe.",
                    user=old_user), 'error'
                )
        else:
            # Prepare one-time system message
    ...
    # Process GET or invalid POST
    return render(user_manager.remove_tenant_user_template, form=form)
...
def delete_tenant():
    ...
    # Initialize form
    form = user_manager.delete_tenant_form(request.form)
    safe_next = _get_safe_next_param(
        'next', user_manager.after_delete_tenant_endpoint)
    form.next.data = safe_next
    ...
    if request.method == 'POST' and form.validate():
        ...
        if db_adapter.UserAuthClass and hasattr(current_user, 'user_auth'):
            ...
            user_auth = current_user
        tenant = [tenant for tenant in user_auth.tenants if tenant.alias == old_tenant][0]  # noqa
        db_adapter.delete_object(tenant)
        db_adapter.commit()

        # Send tenant_added signal
        signals.user_deleted_tenant.send(
            current_app._get_current_object(), user=current_user)

        # Prepare one-time system message
        flash(_(
            "Seu tenant ('%(tenant)s') foi removido.",
            tenant=old_tenant), 'success'
        )
    ...
    # Process GET or invalid POST
    return render(user_manager.delete_tenant_template, form=form)
...
def manage_emails():
    ...
    if request.method == "POST" and form.validate():
        user_emails = db_adapter.add_object(
            db_adapter.UserEmailClass,
...
def _send_registered_email(user, user_email, require_email_confirmation=True):
    ...
    if user_manager.enable_email and user_manager.enable_confirm_email:
        ...
        if user_manager.enable_confirm_email and require_email_confirmation:
            ...
            flash(_(
                'Um email de confirmacao foi enviado para %(email)s '
                'com instrucoes para completar seu cadastro.', email=email
                ), 'success')
        else:
            flash(_('Voce foi cadastrado com sucesso.'), 'success')
...
def _do_login_user(user, safe_next, remember_me=False):
    ...
    if not _call_or_get(user.is_active):
        flash(_('Sua conta não está desbloqueada.'), 'error')
        return redirect(url_for('user.login'))
    ...
    if user_manager.enable_email and user_manager.enable_confirm_email \
       ...
       and not user.has_confirmed_email():
        ...
        flash(_(
            'Seu endereco de email ainda nao foi confirmado. Procure em sua '
            'Caixa de entrada e pasta de Spam pelo email de confirmacao ou '
            '<a href="%(url)s">Reenvie um email de confirmacao</a>.', url=url),
            'error')
        return redirect(url_for('user.login'))
...
def _get_safe_next_param(param_name, default_endpoint):
    if param_name in request.args:
        ...
        safe_next = current_app.user_manager.make_safe_url_function(
            unquote(request.args[param_name]))
    else:
