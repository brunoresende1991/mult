...
class UserManager(object):
    """ This is the Flask-User object that manages
    ...
    def init_app(self, app, db_adapter=None,
                 ...
                 change_theme_form=forms.ChangeThemeForm,
                 add_tenant_form=forms.AddTenantForm,
                 edit_tenant_form=forms.EditTenantForm,
                 add_tenant_user_form=forms.AddTenantUserForm,
                 remove_tenant_user_form=forms.RemoveTenantUserForm,
                 delete_tenant_form=forms.DeleteTenantForm,
                 ...
                 theme_validator=forms.theme_validator,
                 ...
                 change_theme_view_function=views.change_theme,
                 add_tenant_view_function=views.add_tenant,
                 edit_tenant_view_function=views.edit_tenant,
                 add_tenant_user_view_function=views.add_tenant_user,
                 remove_tenant_user_view_function=views.remove_tenant_user,
                 delete_tenant_view_function=views.delete_tenant,
        ...
        self.change_theme_form = change_theme_form
        self.add_tenant_form = add_tenant_form
        self.edit_tenant_form = edit_tenant_form
        self.add_tenant_user_form = add_tenant_user_form
        self.remove_tenant_user_form = remove_tenant_user_form
        self.delete_tenant_form = delete_tenant_form
        ...
        self.change_theme_view_function = change_theme_view_function
        self.add_tenant_view_function = add_tenant_view_function
        self.edit_tenant_view_function = edit_tenant_view_function
        self.add_tenant_user_view_function = add_tenant_user_view_function
        self.remove_tenant_user_view_function = remove_tenant_user_view_function  # noqa
        self.delete_tenant_view_function = delete_tenant_view_function
        ...
        self.resend_confirm_email_view_function = resend_confirm_email_view_function  # noqa
        ...
        blueprint = Blueprint(
            'flask_user', 'flask_user', template_folder='templates')
    ...
    def add_url_routes(self, app):
        """ Add URL Routes"""
        app.add_url_rule(self.login_url,  'user.login',
                         self.login_view_function,  methods=['GET', 'POST'])
        app.add_url_rule(self.logout_url, 'user.logout',
                         self.logout_view_function, methods=['GET', 'POST'])
        app.add_url_rule(self.change_theme_url, 'user.change_theme',
                         self.change_theme_view_function,
                         methods=['GET', 'POST'])
        app.add_url_rule(self.add_tenant_url, 'user.add_tenant',
                         self.add_tenant_view_function,
                         methods=['GET', 'POST'])
        app.add_url_rule(self.edit_tenant_url, 'user.edit_tenant',
                         self.edit_tenant_view_function,
                         methods=['GET', 'POST'])
        app.add_url_rule(self.add_tenant_user_url, 'user.add_tenant_user',
                         self.add_tenant_user_view_function,
                         methods=['GET', 'POST'])
        app.add_url_rule(self.remove_tenant_user_url,
                         'user.remove_tenant_user',
                         self.remove_tenant_user_view_function,
                         methods=['GET', 'POST'])
        app.add_url_rule(self.delete_tenant_url, 'user.delete_tenant',
                         self.delete_tenant_view_function,
                         methods=['GET', 'POST'])
        if self.enable_confirm_email:
            app.add_url_rule(self.confirm_email_url, 'user.confirm_email',
                             self.confirm_email_view_function)
            app.add_url_rule(self.resend_confirm_email_url,
                             'user.resend_confirm_email',
                             self.resend_confirm_email_view_function,
                             methods=['GET', 'POST'])
        if self.enable_change_password:
            app.add_url_rule(self.change_password_url, 'user.change_password',
                             self.change_password_view_function,
                             methods=['GET', 'POST'])
        if self.enable_change_username:
            app.add_url_rule(self.change_username_url, 'user.change_username',
                             self.change_username_view_function,
                             methods=['GET', 'POST'])
        if self.enable_forgot_password:
            app.add_url_rule(self.forgot_password_url, 'user.forgot_password',
                             self.forgot_password_view_function,
                             methods=['GET', 'POST'])
            app.add_url_rule(self.reset_password_url, 'user.reset_password',
                             self.reset_password_view_function,
                             methods=['GET', 'POST'])
        if self.enable_register:
            app.add_url_rule(self.register_url, 'user.register',
                             self.register_view_function,
                             methods=['GET', 'POST'])
        if self.db_adapter.UserEmailClass:
            app.add_url_rule(self.email_action_url,  'user.email_action',
                             self.email_action_view_function)
            app.add_url_rule(self.manage_emails_url, 'user.manage_emails',
                             self.manage_emails_view_function,
                             methods=['GET', 'POST'])
        app.add_url_rule(self.user_profile_url,  'user.profile',
                         self.user_profile_view_function,
                         methods=['GET', 'POST'])
        if self.enable_invitation:
            app.add_url_rule(self.invite_url, 'user.invite',
                             self.invite_view_function,
                             methods=['GET', 'POST'])
    ...
    def get_password(self, user):
        if self.db_adapter.UserAuthClass and hasattr(user, 'user_auth'):
            use_auth_class = True
        else:
            use_auth_class = False
    ...
    def verify_password(self, password, user):
        ...
        In addition, if such password were found, update
        ...
        try:
            verified = passwords.verify_password(
    ...
    def get_user_by_id(self, user_id):
        # Handle v0.5 backward compatibility
        if self.db_adapter.UserAuthClass and self.db_adapter.UserProfileClass:
            ObjectClass = self.db_adapter.UserAuthClass
        else:
    ...
    def find_user_by_id(self, user_id):
        print('Warning: find_user_by_id() will be deprecated in the future.'
              ' User get_user_by_id() instead.')
        return self.get_user_by_id(user_id)

    def get_user_email_by_id(self, user_email_id):
        return self.db_adapter.get_object(
            self.db_adapter.UserEmailClass, user_email_id)

    # NB: This backward compatibility function may be obsoleted in the future
    # Use 'get_user_email_by_id() instead.
    def find_user_email_by_id(self, user_email_id):
        print('Warning: find_user_email_by_id() will be deprecated in the'
              ' future. User get_user_email_by_id() instead.')
        return self.get_user_email_by_id(user_email_id)

    def find_user_by_username(self, username):
        user_auth = None

        # The username field can either be in the UserAuth
        # class or in the User class
        if self.db_adapter.UserAuthClass and \
           hasattr(self.db_adapter.UserAuthClass, 'username'):
            user_auth = self.db_adapter.ifind_first_object(
                self.db_adapter.UserAuthClass, username=username)
            ...
            user = self.db_adapter.ifind_first_object(
                self.db_adapter.UserClass, username=username)

        return user

    def find_tenant(self, tenant):
        return self.db_adapter.ifind_first_object(
            self.db_adapter.Tenant, alias=tenant)

    def find_user_by_email(self, email):
        user_email = None
        user_auth = None
        if self.db_adapter.UserEmailClass:
            user_email = self.db_adapter.ifind_first_object(
                self.db_adapter.UserEmailClass, email=email)
            user = user_email.user if user_email else None
        else:
            # The email field can either be in the
            # UserAuth class or in the User class
            if self.db_adapter.UserAuthClass and \
               hasattr(self.db_adapter.UserAuthClass, 'email'):
                user_auth = self.db_adapter.ifind_first_object(
                    self.db_adapter.UserAuthClass, email=email)
                ...
                user = self.db_adapter.ifind_first_object(
                    self.db_adapter.UserClass, email=email)
    ...
    def username_is_available(self, new_username):
        """ Return True if new_username does not exist or if
            new_username equals old_username.
            Return False otherwise."""
        # Allow user to change username to the current username
        if _call_or_get(current_user.is_authenticated):
            if self.db_adapter.UserAuthClass and \
               hasattr(current_user, 'user_auth'):
                current_username = current_user.user_auth.username
            else:
                current_username = current_user.username
            if new_username == current_username:
                return True
        # See if new_username is available
        return self.find_user_by_username(new_username) is None

    def tenant_is_available(self, new_tenant):
        """ Return True if new_tenant does not exist or
            if new_tenant equals old_tenant.
            Return False otherwise."""
        # Allow tenant to verify availability
        if _call_or_get(current_user.is_authenticated):
            if new_tenant in current_user.tenants:
                return True
        # See if new_tenant is available
        return self.find_tenant(new_tenant) is None
    ...
    def send_reset_password_email(self, email):
        ...
        if user:
            ...
            emails.send_forgot_password_email(
                user, user_email, reset_password_link)

            # Send forgot_password signal
            signals.user_forgot_password.send(
